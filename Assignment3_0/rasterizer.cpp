...
void rst::rasterizer::rasterize_triangle(
  ...
  // Step 1: Find out the bounding box of current triangle.
    float min_x_f = std::min({ t.v[0].x(), t.v[1].x(), t.v[2].x() });
    float max_x_f = std::max({ t.v[0].x(), t.v[1].x(), t.v[2].x() });
    float min_y_f = std::min({ t.v[0].y(), t.v[1].y(), t.v[2].y() });
    float max_y_f = std::max({ t.v[0].y(), t.v[1].y(), t.v[2].y() });

    int min_x = (int)std::floor(min_x_f);
    int max_x = (int)std:: ceil(max_x_f);
    int min_y = (int)std::floor(min_y_f);
    int max_y = (int)std:: ceil(max_y_f);

  // Step 2: Iterate through pixels, find if inside the triangle.
    for (int x = min_x; x <= max_x; x++) {
        for (int y = min_y; y <= max_y; y++) {
            ...
            if (!insideTriangle(px, py, t.v)) {
                ...
                // Step 2.1: If true, we define the following variables:
                ...
                // Step 2.1.1: Check the zp with the depth buffer value (from Asst2)
                ...
                // Step 2.1.4:
                // Set the current pixel to the color of the triangle.
