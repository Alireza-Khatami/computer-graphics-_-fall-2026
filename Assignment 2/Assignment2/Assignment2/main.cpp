...
int main(int argc, const char** argv) {
  ...
  // Cube mode: ./Rasterizer cube <blend_mode> <output_file>
  if (argc >= 2 && std::string(argv[1]) == "cube") {
    int blend_mode_arg = (argc >= 3) ? std::stoi(std::string(argv[2])) : 0;
    filename = (argc >= 4) ? std::string(argv[3]) : "cube.png";
    r.clear(rst::Buffers::Color | rst::Buffers::Depth);
    cube_scene.render_with(r, blend_mode_arg);
