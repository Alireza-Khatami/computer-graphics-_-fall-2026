...
class Sphere : public Object {
 public:
  ...
  bool intersect(const Vector3f& orig, const Vector3f& dir, float& tnear,
      ...
      // If the closest intersection is behind the ray origin, try the farther one
      if (t0 < 0.0f) {
          t0 = t1;
          if (t0 < 0.0f)
              return false;  // both behind the origin → no valid hit
      }

      tnear = t0;  // nearest positive intersection
      return true;
  }
