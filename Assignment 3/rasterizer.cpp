...
// {
    /*
     * TODO: Implement inside-triangle test using the edge cross-product method.
     *
     * Step 1:
     *   Extract the 2D screen-space coordinates of the three triangle vertices
     *   from _v (ignore z and w).
     *
     * Step 2:
     *   Construct 2D edge vectors:
     *     e0 = v1 - v0
     *     e1 = v2 - v1
     *     e2 = v0 - v2
     *
     * Step 3:
     *   Construct vectors from each vertex to the pixel center:
     *     p0 = P - v0
     *     p1 = P - v1
     *     p2 = P - v2
     *   where P = (x + 0.5, y + 0.5).
     *
     * Step 4:
     *   Compute the 2D cross product (z-component only) for:
     *     c0 = e0 × p0
     *     c1 = e1 × p1
     *     c2 = e2 × p2
     *
     * Step 5:
     *   Check the signs of c0, c1, c2:
     *     - If all are positive, the point is inside.
     ...
     *   cross product z-value = x1*y2 - y1*x2.
     */
...
void rst::rasterizer::rasterize_triangle(
    ...
    int min_x = std::floor(std::min({ t.v[0].x(), t.v[1].x(), t.v[2].x() }));
    ...
    for (int x = min_x; x <= max_x; ++x) {
        for (int y = min_y; y <= max_y; ++y) {
            ...
            // Step 2.1: Interpolate depth values
            float Z = 1.0f / (alpha / t.v[0].w() + beta / t.v[1].w() + gamma / t.v[2].w());
            float z_depth_interpolated = alpha * t.v[0].z() / t.v[0].w() +
                beta * t.v[1].z() / t.v[1].w() +
                gamma * t.v[2].z() / t.v[2].w();
            z_depth_interpolated *=  Z;
            // Step 2.1.1: Depth buffer check
            int idx = get_index(x, y);
            if (depth_buf[idx] >= z_depth_interpolated) {
                depth_buf[idx] = z_depth_interpolated;
                // Step 2.1.2: Interpolate attributes
                // Eigen::Vector3f interpolated_color = interpolate(alpha, beta, gamma,
                //     t.color[0], t.color[1], t.color[2], 1);
                // Eigen::Vector3f interpolated_normal = interpolate(alpha, beta, gamma,
                //     t.normal[0], t.normal[1], t.normal[2],1);
                // Eigen::Vector2f interpolated_texcoords = interpolate(alpha, beta, gamma,
					      //                                                     t.tex_coords[0],
                //                                                     t.tex_coords[1],
                //                                                     t.tex_coords[2],Z,t);
                // Eigen::Vector3f interpolated_viewpos = interpolate(alpha, beta, gamma,
                ...
                // Step 2.1.4: Set the pixel in the framebuffer
