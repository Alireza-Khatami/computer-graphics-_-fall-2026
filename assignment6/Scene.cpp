...
Vector3f Scene::castRay(const Ray &ray) const {
    ...
    // === Specular (Glass / Mirror) — delta BSDF, no direct lighting ===
    if (hit.m->isSpecular()) {
        if (get_random_float() >= RussianRoulette) return Vector3f(0.0f);
        Vector3f wi = hit.m->sample(wo, N);
        // Offset origin to avoid self-intersection: move along/against normal
        // depending on whether the new ray exits or enters the surface
