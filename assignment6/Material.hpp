...
class Material {
 ...
 public:
  ...
  // importance sample a ray by Material properties
  // given a ray, calculate the PdF of this ray
  inline float pdf(const Vector3f &wi, const Vector3f &wo, const Vector3f &N);
  // importance sample pdf
...
Vector3f Material::sample(const Vector3f &wi, const Vector3f &N) {
  switch (m_type) {
    ...
    case GLASS: {
      // wi is the outgoing direction (toward camera); incident direction is -wi
      float kr;
      fresnel(-wi, N, ior, kr);
      if (get_random_float() < kr) {
        ...
        // refract; fall back to reflection on total internal reflection
        Vector3f refracted = refract(-wi, N, ior);
        if (refracted.norm() < EPSILON) return reflect(-wi, N).normalized();
...
float Material::pdf(const Vector3f &wi, const Vector3f &wo, const Vector3f &N) {
  switch (m_type) {
    ...
    case GLASS: {
      // Delta distribution: the sampled direction is exact, handled in castRay
      return 1.0f;
    }
  }
  return 0.0f;
...
Vector3f Material::eval(const Vector3f &wi, const Vector3f &wo,
  ...
  switch (m_type) {
    ...
    case GLASS: {
      // The Fresnel weight is baked into sample(); contribution is 1 per path
