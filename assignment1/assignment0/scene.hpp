...
// Common interface for every scene in this assignment.
// main.cpp keeps one instance of each scene alive and switches between them
// with the keyboard, so a scene owns its own state (camera, angles, ...).
class Scene {
 public:
  virtual ~Scene() = default;

  // Short name shown in the console / window title.
